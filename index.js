...
document.addEventListener("click", function (e) {
  if (e.target.dataset.like) {
    ...
    handleDeleteClick(e.target.dataset.deleted);
  } else if (e.target.id === "tweet-btn") {
    handleTweetBtnClick();
  } else if (e.target.dataset.post) {
    handleReplyTweetClick(e.target.dataset.post);
  }
});
...
function handleReplyTweetClick(tweetID) {
  const replyText = document.getElementById(`reply-text-${tweetID}`);
  const targetTweetObj = tweetsData.filter(function (tweet) {
    return tweet.uuid === tweetID;
  })[0];
...
<div class="tweet">
    ...
    <div class="hidden" id="replies-${tweet.uuid}">
        ...
        <div class="tweet-reply">
            ...
            ></textarea>
            <button class="reply-btn" data-post="${tweet.uuid}">Post reply</button>
        </div>
    </div>   
</div>
`;
